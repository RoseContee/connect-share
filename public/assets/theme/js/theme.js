...
(function (global, factory) {
  ...
  var Layout = /*#__PURE__*/function () {
    ...
    _proto.fixLayoutHeight = function fixLayoutHeight(extra) {
      ...
      var $orgchartSelector = $__default["default"]('#orgchart');
      var contentHeight;

      if (offset !== false) {
        if (max === heights.controlSidebar) {
          contentHeight = max + offset;
          $contentSelector.css(this._config.panelAutoHeightMode, contentHeight);
          $orgchartSelector.css(this._config.panelAutoHeightMode, `calc(${contentHeight}px - 4.5rem - 56px)`)
        } else if (max === heights.window) {
          contentHeight = max + offset - heights.header - heights.footer;
          $contentSelector.css(this._config.panelAutoHeightMode, contentHeight);
          $orgchartSelector.css(this._config.panelAutoHeightMode, `calc(${contentHeight}px - 4.5rem - 56px)`)
        } else {
          contentHeight = max + offset - heights.header;
          $contentSelector.css(this._config.panelAutoHeightMode, contentHeight);
          $orgchartSelector.css(this._config.panelAutoHeightMode, `calc(${contentHeight}px - 4.5rem - 56px)`)
        }

        if (this._isFooterFixed()) {
          contentHeight = parseFloat($contentSelector.css(this._config.panelAutoHeightMode)) + heights.footer;
          $contentSelector.css(this._config.panelAutoHeightMode, contentHeight);
          $orgchartSelector.css(this._config.panelAutoHeightMode, `calc(${contentHeight}px - 4.5rem - 56px)`)
        }
      }
